...
<script>
    ...
    try {
        ...
        // 주소 정보 조회 쿼리 실행
        String sql = "SELECT * FROM address_table WHERE 1=1";
        if (!location.isEmpty()) {
            sql += " AND location = ?";
        }
        if (!time.isEmpty()) {
        ...
        if (!height_limit.isEmpty()) {
            sql += " AND height_limit = ?";
        }
        ...
        if (!location.isEmpty()) {
            pstmt.setString(parameterIndex++, location);
        }
        if (!time.isEmpty()) {
            pstmt.setString(parameterIndex++, time);
        }
        if (!height_limit.isEmpty()) {
            pstmt.setString(parameterIndex++, height_limit);
        }
        if (!vehicle_type.isEmpty()) {
            pstmt.setString(parameterIndex++, vehicle_type);
        }
